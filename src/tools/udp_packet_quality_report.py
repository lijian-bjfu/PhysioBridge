...
try:
    ...
    Tk, filedialog, plt = None, None, None
    _PLT_OK = False
...
def find_latest_session_dir(root_log_dir: Path) -> Optional[Path]:
    """
    在指定的日志根目录中，寻找并返回最新修改的会话子目录。
    """
    if not root_log_dir.is_dir():
        print(f"[err] 日志根目录不存在: {root_log_dir}")
        return None

    # 获取所有子目录，并按修改时间降序排列
    sub_dirs = sorted(
        [d for d in root_log_dir.iterdir() if d.is_dir()],
        key=os.path.getmtime,
        reverse=True
    )

    if not sub_dirs:
        print(f"[err] 在 {root_log_dir} 中找不到任何会话目录。")
        return None

    latest_dir = sub_dirs[0]
    print(f"[*] 自动选择最新会话目录: {latest_dir.name}")
    return latest_dir

def pick_dir_dialog(title: str, initial: Path) -> Optional[Path]:
    """弹出一个对话框让用户选择目录。"""
    if Tk is None or filedialog is None:
        print("[err] 未能加载tkinter图形界面库，无法弹出选择框。")
        return None
...
def main():
    ...
    # ap.add_argument("session_name", help="要分析的会话文件夹名称 (例如: S20250905-153000)")
    # args = ap.parse_args()
    ...
    if not session_dir:
        print("[*] 未在命令行指定会话目录，将弹出选择窗口...")
        session_dir = pick_dir_dialog(
            title="请选择要分析的会话文件夹 (例如: S20250905-153000)",
            initial=RECORDER_DATA_DIR
        )
     # 如果用户取消了选择，则退出
    if not session_dir:
        return

    # 2. 在指定的目录中寻找 .metrics.jsonl 文件，并赋值给【mpath】
    metrics_files = list(session_dir.glob('*.metrics.jsonl'))
    if not metrics_files:
        print(f"[err] 在目录 {session_dir} 中找不到任何 *.metrics.jsonl 日志文件。")
        return
    mpath = metrics_files[0] 

    # 3. 确定输出路径，并赋值给【out】
    out = session_dir / mpath.with_name(mpath.stem.replace(".metrics", "") + "_udp_quality.md")
