...
polar 链路为：Polar 测量心率数据 → iPhone App获取数据 → UDP 发包 → 局域网 → 桥接器/`polar_bridge.py` → 在本机发布两路 LSL → LabRecorder 发现并录制 → `.xdf` 文件 →  `.csv` 文件。呼吸带链路为：呼吸带设备测量呼吸频率 → 桥接器/`HKH_bridge.py` → 在本机发布 LSL → LabRecorder 发现并录制 → `.xdf` 文件 →  `.csv` 文件。
