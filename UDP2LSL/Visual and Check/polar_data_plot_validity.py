...
def main():
    ...
    if "verity" in data.get("PPG", {}):
        ...
        grades.append(g)
        plot_ppg(t, X, root / f"ppg_{dev}.png", marks, fs, dev)
    ...
    if "ACC" in data:
        for device, (t, X, _) in data["ACC"].items():
            ...
            grades.append(g)
            plot_acc(t, X, root / f"acc_{device}.png", fs, device)
    ...
    if "h10" in data.get("HR", {}) and "h10" in data.get("RR", {}):
        ...
        if overlap_end > overlap_start:
            ...
            if reason: report.append(f"[HR vs RR] -> N/A | 原因: {reason}")
                ...
                grades.append(g)
                plot_hr_overlay(t_hr, X_hr[:,0], t_rr_evt, X_rr[:,0], root/f"hr_rr_overlay_h10.png", "RR", "h10", "h10")
        ...
        # 绘制新的RR图表
        plot_interval_tachogram(t_rr, X_rr[:, 0], root / "rr_tachogram_h10.png", "RR", "h10")
        plot_poincare(X_rr[:, 0], root / "rr_poincare_h10.png", "RR", "h10")
    ...
    if "verity" in data.get("PPI", {}):
        ...
        plot_ppi_quality(t_ppi, hdr_ppi, X_ppi, root / f"ppi_quality_verity.png", "verity")
        ...
        if "verity" in data.get("HR", {}):
            ...
            if overlap_end > overlap_start:
                ...
                if reason: report.append(f"[HR vs PPI] -> N/A | 原因: {reason}")
                    ...
                    grades.append(g)
                    plot_hr_overlay(t_hr, X_hr[:,0], t_ppi_evt, X_ppi[:,0], root/f"hr_ppi_overlay_verity.png", "PPI", "verity", "verity")
